...
| Policy Engine | Guardian MGS v3.5.1 | Managed service — no Docker, no infra |
...
Each of Eggologic's three value propositions has a distinct technical flow through the system:

### Restaurants → SCS Environmental Certification

```
Restaurant delivers waste
        │
        ▼
Project_Proponent submits Waste Delivery (VC)
        │
        ▼
VVB approves → EGGOCOIN minted → delivery logged on-chain
        │
        ▼
System tracks: Σ(kg_ajustados), avg quality grade, delivery frequency, consecutive months
        │
        ▼
Thresholds met? → Certification VC issued on Hedera
  • Bronze: 3+ months, 100+ kg, Grade B avg
  • Silver: 6+ months, 500+ kg, Grade A/B avg, Alliance status
  • Gold:   12+ months, 1,000+ kg, Grade A avg, linked to CIN
        │
        ▼
Restaurant displays verifiable badge → links to HashScan proof
```

**Hedera services used:** HCS (delivery VCs, certification VCs), HTS (EGGOCOIN mint), Mirror Node (verification)

### Consumers → Real Food Traceability

```
Consumer scans QR code on egg carton / compost bag
        │
        ▼
Dashboard loads product Trust Chain from Guardian + Mirror Node
        │
        ▼
Displays full origin chain:
  ├─ Restaurant: "El Gaucho" delivered 50 kg waste on 2026-03-15
  ├─ BSF Batch: BATCH-042, processed 2026-03-16 to 2026-03-23
  ├─ Production: 45 eggs produced on 2026-03-24
  └─ Impact: contributed to CIN #7 (0.35 tCO₂e portion)
        │
        ▼
Consumer verifies on HashScan → immutable, public, no trust required
```

**Hedera services used:** HCS (Trust Chain VCs), HTS (EGGTOKEN Phase 3), Mirror Node (public verification)

### Companies → ESG Compliance & Carbon Reporting

```
Company sponsors waste diversion or purchases CIN directly
        │
        ▼
Waste deliveries accumulate → Impact Calculation submitted by OWNER
        │
        ▼
VVB reviews: verifies delivery VCs, carbon formula, conservative factor
        │
        ▼
VVB approves → CIN NFT minted (1 CIN = 1 tCO₂e avoided)
        │
        ▼
CIN transferred to company's Hedera account
        │
        ▼
Company uses CIN in ESG reporting:
  ├─ GRI 305-5: emission reduction quantity + methodology
  ├─ GRI 306-4: waste diverted from disposal (kg records)
  ├─ TCFD Metrics: Scope 3 Cat 5 avoidance
  └─ Audit trail: HashScan link for independent verification
```

**Hedera services used:** HCS (Impact VCs, VVB approval), HTS (CIN NFT mint + transfer), Mirror Node (audit trail)

### Token-to-Value Summary

| Token | Created By | Consumed By | Hedera Service | Value Delivered |
|---|---|---|---|---|
| EGGOCOIN | VVB approval of delivery | Restaurant (redeem/certify) | HTS fungible | SCS Environmental Certification progress |
| CIN NFT | VVB approval of impact | Company (ESG report) | HTS NFT | Auditable carbon credit |
| EGGTOKEN (Phase 3) | Production record | Consumer (QR scan) | HTS fungible | Food traceability |
